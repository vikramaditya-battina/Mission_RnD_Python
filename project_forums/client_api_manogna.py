...
def signup():
    ...
    obj.cnf=raw_input("cofirm password")
...
def display_options(cname,fname,uname):
    ...
    if choice=='1':
        ...
        qid=raw_input("\nselect a question:")
        postcomment(cname,fname,uname,qid)
    elif choice=='3':
        qid=raw_input("\nselect a question:")
        viewcomments(cname,fname,uname,qid)
    elif choice=='4':
        fname=raw_input("\nenter forum name:")
        viewquestions(cname,fname,uname)
    elif choice=='5':
...
def postcomment(cname,fname,uname,qid):
    cons.set_text_attr(cons.FOREGROUND_MAGENTA  |
                       cons.FOREGROUND_INTENSITY)
    obj=json()
...
def viewcomments(cname,fname,uname,qid):
    obj=json()
